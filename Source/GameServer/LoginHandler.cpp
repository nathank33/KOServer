...
void CUser::LoginProcess(Packet & pkt) {
	...
	//printf("strAcc  : %s strPass : %s\n",strAccountID.c_str(),strPasswd.c_str());

	if (strAccountID.empty() || strAccountID.size() > MAX_ID_SIZE || strPasswd.empty() || strPasswd.size() > MAX_PW_SIZE)
		goto fail_return;
