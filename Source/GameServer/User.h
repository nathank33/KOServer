...
// Time (in seconds) between each skill request (-1 sec). 2 le çarpýldý
...
public:
	...
	time_t	lastArrangeTime;

	uint8	m_bAccountStatus;

	bool CheckGiveSlot(uint8 sSlot);

	void SendRepurchase(bool nReflash = false);
	void RecvRepurchase(Packet& pkt);
	// nSlot | nTime | nItemID
	typedef std::map<uint32, _ITEM_REPURCHASE *> RepurchaseMap;
	...
	SkillCooldownList	m_CastingCoolDownList;
	...
	time_t	ChangeCoverTitleRequest;
	time_t	ChangeSkillTitleRequest;
	time_t	ResetCoverTitleRequest, ResetSkillTitleRequest;
...
public:
	...
	INLINE bool isInSameParty(CUser * pTarget) { return m_sPartyIndex == pTarget->m_sPartyIndex && m_sPartyIndex != uint16(-1); }
	...
	INLINE bool isLockableScroll(uint8 buffType) { return (buffType == BUFF_TYPE_HP_MP || buffType == BUFF_TYPE_AC || buffType == BUFF_TYPE_FISHING || buffType == BUFF_TYPE_DAMAGE || buffType == BUFF_TYPE_SPEED || buffType == BUFF_TYPE_STATS || buffType == BUFF_TYPE_BATTLE_CRY || buffType == BUFF_TYPE_VARIOUS_EFFECTS); }
	...
	*/
	...
	void AchieveGetItem(Packet & pkt);
	void LoadAchievementData(Packet & pkt);
	void AchieveTimeQuest(Packet & pkt);
	void SendAchievementDetails();
	void ChangeCoverTitle(Packet & pkt);
	void ChangeSkillTitle(Packet & pkt);
	void CoverTitleReset();
	void SkillTitleReset();
	void SendAchieveStatus(uint16 AchieveID, uint8 AchieveStatus);
	void AchieveType1(uint8 pType);
	void AchieveType2(CNpc * pNpc);
	void AchieveType3();
	void AchieveType4(uint8 pType);
	...
	bool CheckExistSpecialItemAnd(int32 nItemID1, int16 sCount1, int32 nItemID2, int16 sCount2,
	...
	CUser * GetItemRoutingUser(uint32 nItemID, uint16 sCount, _LOOT_BUNDLE * pBundle);
	...
	void SendStackChangeSpecial(uint32 nItemID, uint32 nCount /* needs to be 4 bytes, not a bug */, uint16 sDurability, uint8 bPos, bool bNewItem = false, uint32 Time = 0, _ITEM_DATA *pItem = nullptr);
	...
	void NationChange(Packet & pkt);
	void SendNationChange();
	uint8 GetNewRace();
	void GenderChange(Packet & pkt);
	void HandlePet(Packet & pkt);
	void KillMyPet();
	...
	COMMAND_HANDLER(HandleBotSummonCommand);
	...
	bool CheckSpecialExchange(int nExchangeID);
	...
	void CancelMerchant();
	void OfficialList(Packet & pkt);
	void ListSend(Packet & pkt);
	void ListMoveProcess(Packet & pkt);
	uint8 MerchantType;
	// buying merchants
	...
	void ItemUpgradePet(Packet & pkt);
	...
	void ItemDisassembling(Packet & pkt);
	void ItemSealProcess(Packet & pkt);
	void SealItem(uint8 bSealType, uint8 bSrcPos);
	void CharacterSealProcess(Packet & pkt);
	void CharacterGetSealed(Packet & pkt);
	void CharacterGetUnSealed(Packet & pkt);
	void SendCharacterSealProcess();
	void SendCharacterSealInfo(Packet & pkt);
	...
	void HandleCNameChange(Packet & pkt);
	void HandlePlayerNameChange(Packet & pkt);
	void HandleKnightsNameChange(Packet & pkt);

	void SendNameChange(NameChangeOpcode opcode = NameChangeShowDialog);
	void SendKnightsNameChange(ClanNameChangeOpcode opcode = ClanNameChangeShowDialog);

	void HandleHelmet(Packet & pkt);
	void HandleCapeChange(Packet & pkt);
	void HandleVIPStorage(Packet & pkt);
	void VipStorageOpenRequest(Packet & pkt);
	void VipStorageItemInput(Packet & pkt);
	void VipStorageItemOutput(Packet & pkt);
	void VipStorageItemMove(Packet & pkt);
	void VipStorageInventoryItemMove(Packet & pkt);
	void VipStorageUseKey(Packet & pkt);
	void VipStorageChangePassword(Packet & pkt);
	void VipStorageNewPassword(Packet & pkt);
	void VipStorageOpen(Packet & pkt);
	...
	void HandleMiningThing(Packet & pkt);
	...
	void ZoneChange(uint16 sNewZone, float x, float z, uint16 nEventRoom = 0);
	...
	void ApplyAchieveSkillBonuses(uint16 pAchieveSkill, uint8 eNum);
	void SendTime();
	void SendWeather();
	void GenieInfo();
	...
	void LogOut();
	void SendMyInfo();
	void SetSpecialItemData(_ITEM_DATA * pSlot, ByteBuffer & result);
	void SetExchangeSpecialItemData(_EXCHANGE_ITEM * pSlot, ByteBuffer & result);
	void SetMerchantSpecialItemData(_MERCH_DATA * pSlot, ByteBuffer & result);
	...
	void NationChangeHandle(Packet & pkt);
